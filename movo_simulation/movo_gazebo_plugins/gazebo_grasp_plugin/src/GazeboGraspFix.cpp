...
void GazeboGraspFix::OnUpdate() {
    ...
    for (gripCntIt = this->gripCounts.begin(); gripCntIt != this->gripCounts.end(); ++gripCntIt){
        ...
        for (pointIt=initColls.begin(); pointIt!=initColls.end(); ++pointIt)
        {
            ...
            ignition::math::Pose3d currObjWorldPose=cpInfo.collObj->GetLink()->GetWorldPose();

            // get world pose of link
            ignition::math::Pose3d currLinkWorldPose=cpInfo.collLink->GetLink()->GetWorldPose();

            // Get transform for currLinkWorldPose as matrix
            ignition::math::Matrix4d worldToLink=currLinkWorldPose.rot.GetAsMatrix4();
            ...
            ignition::math::Vector3d currContactWorldPose=_currContactWorldPose.GetTranslation();
...
bool GazeboGraspFix::checkGrip(const std::vector<ignition::math::Vector3d>& forces, float minAngleDiff, float lengthRatio){
    ...
    for (it1=forces.begin(); it1!=forces.end(); ++it1){
        ...
        for (it2=it1+1; it2!=forces.end(); ++it2){
            ...
            /*ignition::math::Vector3d _v1=v1;
            ignition::math::Vector3d _v2=v2;
            _v1/=l1;
            _v2/=l2;
...
void GazeboGraspFix::OnContact(const ConstContactsPtr &_msg)
{
    ...
    for (int i = 0; i < _msg->contact_size(); ++i) {
        ...
        if ((collision1 && !collision1->IsStatic()) && (collision2 && !collision2->IsStatic()))
        {
            ...
            ignition::math::Pose3d linkWorldPose=linkCollision->GetLink()->GetWorldPose();

            // To find out the collision point relative to the Link's local coordinate system, first get the Poses as 4x4 matrices
            ignition::math::Matrix4d worldToLink=linkWorldPose.rot.GetAsMatrix4();
            ...
            ignition::math::Vector3d contactPosInLocal = contactInLocal.GetTranslation();
            
            //std::cout<<"---------"<<std::endl;    
            //std::cout<<"CNT in loc: "<<contactPosInLocal.x<<","<<contactPosInLocal.y<<","<<contactPosInLocal.z<<std::endl;

            /*ignition::math::Vector3d sDiff=avgPos-linkWorldPose.pos;
            ...
            ignition::math::Vector3d lX=linkWorldPose.rot.GetXAxis();    
            ignition::math::Vector3d lY=linkWorldPose.rot.GetYAxis();    
            ignition::math::Vector3d lZ=linkWorldPose.rot.GetZAxis();    
            ...
            ignition::math::Pose3d objWorldPose = objCollision->GetLink()->GetWorldPose();
            ignition::math::Matrix4d worldToObj = objWorldPose.rot.GetAsMatrix4();
            worldToObj.SetTranslate(objWorldPose.pos);
    
            ignition::math::Matrix4d objInLocal = worldToLinkInv * worldToObj;
            ignition::math::Vector3d objPosInLocal = objInLocal.GetTranslation();
