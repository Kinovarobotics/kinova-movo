...
namespace range_sensor_filters
{
    ...
    public:
        ...
        bool configure() 
        {
            ...
            getParam("base_frame", base_frame_);
            if (base_frame_.empty()) 
            {
                ROS_WARN("No base frame provided, assuming footprint is in the frame of reference of the sensor.");
            ...
            else 
            {
                ...
                std::string resolved_frame = tf::resolve(tf_prefix, base_frame_);
                ROS_INFO("Resolved footprint frame from %s to %s.", base_frame_.c_str(), resolved_frame.c_str());
                base_frame_ = resolved_frame;
            }
        ...
        bool update(const sensor_msgs::LaserScan& input_scan, sensor_msgs::LaserScan& filtered_scan)
        {
            ...
            if (get_transformation_) 
            {
                try 
                {
                    ...
                    listener_->waitForTransform(input_scan.header.frame_id, base_frame_, ros::Time(0), ros::Duration(10.0) );
                    listener_->lookupTransform(input_scan.header.frame_id, base_frame_, ros::Time(0), transform);
                    for (uint32_t i = 0; i < footprint_.size(); ++i) 
                    {
                        footprint_[i] = transform * footprint_[i]; 
                    }
                    ROS_INFO("Transformation from %s to %s received. Transforming footprint", base_frame_.c_str(), input_scan.header.frame_id.c_str());
                } 
