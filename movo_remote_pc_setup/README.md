...
**Important note: Users of movo with 6 dof arms, if you decide to update your code base on your movo robot (if you answer 'yes' to the question "Do you want to update the robot now?\nIf not the code from movo2 will overwrite the src directory....\n" when running the setup_remote_pc script), do not forget to change the movo configuration file (movo_config.bash) located in movo_common/movo_config/movo_config.bash. Set MOVO_HAS_KINOVA_ARM_7DOF=false
and MOVO_HAS_KINOVA_ARM_6DOF=true instead of MOVO_HAS_KINOVA_ARM_7DOF=true and MOVO_HAS_KINOVA_ARM_6DOF=false. The config file MUST be set that way on all three computers: your remote computer, movo1 and movo2.**
